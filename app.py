...
from components.blog_display import render_blog
from styles.custom_css import load_css
from utils.history_utils import save_to_history  # ✅ added
...
def main():
    ...
    # Sidebar (config + history display)
    config, history = render_sidebar()
    ...
    audience = st.selectbox(
        "Target Audience",
        ["Researchers", "Data Scientists", "Common People", "Students", "Business Professionals"]
    )
    word_count = st.slider("Word Count", 100, 2000, 500, 50)
    tone = st.select_slider(
        "Tone",
        options=["Formal", "Professional", "Conversational", "Casual", "Friendly"],
        value="Professional"
    )
    ...
    if st.button("🚀 Generate Blog"):
        if not topic:
            ...
            with st.spinner("Generating blog..."):
                ...
                # ✅ Save blog to history.json
                save_to_history(topic, blog_content, audience, word_count, tone)
